...
await db.write({
  ...
  statements: [{ sql: "INSERT INTO kv VALUES ('greeting', 'hello')" }],
});
...
The published package includes the prebuilt addon and Go shared library for
`linux-x64` and `linux-arm64`. Both files must be present for the host
platform. Unsupported platforms must build from source with Go and CGO
enabled.
