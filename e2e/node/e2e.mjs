// walrusd end-to-end check for the Node.js binding (@walrusd/db) against
// real object storage (MinIO/S3) and a real Redis/Valkey lease store.
//
// The full loop for one fresh database: a read before any write (no lease),
// a schema write, an insert write, an idempotent retry, and read-after-write.
// Around every step the lease key is inspected directly in Redis (plain RESP
// over a socket, no client library) to prove the runtime alone acquires and
// releases the lease and that reads never touch it.
//
// Run (after `cd bindings/node && npm run build`):
//
//	node e2e/node/e2e.mjs
//
...
import { WalrusdDatabase } from "../../bindings/node/dist/index.js";
...
const databaseID = `users/node_${Math.random().toString(36).slice(2)}`;
...
async function main() {
  const db = new WalrusdDatabase({ owner: "e2e-node-owner", redisAddress: REDIS });
  try {
...
async function redis(addr, command) {
...
function parseReply(buffer) {
  ...
  if (header.startsWith("$")) {
    ...
    return { value: buffer.subarray(end + 2, end + 2 + length).toString() };
  }
  throw new Error(`redis: unsupported reply ${header}`);
}

async function leaseState(step) {
  const fields = await redis(REDIS, `HGET ${leaseKey} data`);
  assert(fields, `${step}: lease record ${leaseKey} not found`);
  return JSON.parse(fields);
}

async function requireLeaseAbsent(step) {
  const exists = await redis(REDIS, `EXISTS ${leaseKey}`);
  assert(exists === 0, `${step}: lease key ${leaseKey} exists; the operation took a lease it should not have`);
}

async function requireLeaseReleased(step) {
  const record = await leaseState(step);
  assert(record.state === "released", `${step}: lease state ${record.state}, want released`);
}

async function requireLeaseUnchanged(step, before) {
